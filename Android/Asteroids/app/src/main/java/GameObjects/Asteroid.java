...
/**
 * Class that implements an Asteroid for the touch game mode
 */
public class Asteroid extends GameObject {
    ...
    // to controlo movement depending on game type
    private int type;
    ...
    public Asteroid(float x, float y, int type) {
        ...
        if (type == SMALL) {
            numPoints = 8;
            width = height = (int) Asteroids.HEIGHT/40;
            speed = rand.nextFloat() *
                    (Asteroids.HEIGHT/6.85f - Asteroids.HEIGHT/9.6f) + Asteroids.HEIGHT/6.85f;
            score = 100;
        }
        else if (type == MEDIUM) {
            numPoints = 10;
            width = height = (int) Asteroids.HEIGHT/24;
            speed = rand.nextFloat() *
                    (Asteroids.HEIGHT/12 - Asteroids.HEIGHT/16) + Asteroids.HEIGHT/12;
            score = 50;
        }
        else if (type == LARGE) {
            numPoints = 12;
            width = height = (int) Asteroids.HEIGHT/12;
            speed = rand.nextFloat() *
                    (Asteroids.HEIGHT/24 - Asteroids.HEIGHT/48) + Asteroids.HEIGHT/24;
            score = 20;
        }

        // movement controlers
        rotationSpeed = rand.nextFloat() * 2 - 1;
        radians = rand.nextFloat() * (Asteroids.HEIGHT/240 * 3.1415f)
                + Asteroids.HEIGHT/240 * 3.1415f;
     ...
     * @return      Asteroid
     */
    public Asteroid(float x, float y, float dx, float dy, int type) {
