...
/**
 * Class that implements a ship that is controled by the player for the touch game mode
 */
public class Player extends GameObject {
    ...
    public Player(ArrayList<Bullet> bullets) {
        ...
        maxSpeed = Asteroids.HEIGHT;
        acceleration = Asteroids.HEIGHT*2/3;
        deceleration = Asteroids.HEIGHT/20;
     ...
     * @param tx postion in the canvas x edge where the touch has been made
     * @param ty postion in the canvas y edge where the touch has been made
     */
    public void touch(float tx, float ty) {
        radians = (float)Math.atan((y-ty)/(x-tx));
        if (tx < x) radians += 3.1415f;
        shoot();
    }

    /**
     * Handles drag events
     * @param tx postion in the canvas x edge where the drag has been made
     * @param ty postion in the canvas y edge where the drag has been made
     */
    public void drag(float tx, float ty) {
        drag = true;
        radians = (float)Math.atan((y-ty)/(x-tx));
        if (tx < x) radians += 3.1415f;
    }
